...
function networkUp() {
  ...
  if [ ! -d "crypto-config" ]; then
    generateCerts
    replacePrivateKey
    replaceBlockChainExplorer
    generateChannelArtifacts
  fi
  COMPOSE_FILES="-f ${COMPOSE_FILE}"
  if [ "${CERTIFICATE_AUTHORITIES}" == "true" ]; then
    COMPOSE_FILES="${COMPOSE_FILES} -f ${COMPOSE_FILE_CA}"
    export BYFN_ACADEMY_PRIVATE_KEY=$(cd crypto-config/peerOrganizations/academy.certificate.com/ca && ls *_sk)
    export BYFN_STUDENT_PRIVATE_KEY=$(cd crypto-config/peerOrganizations/student.certificate.com/ca && ls *_sk)
  fi
...
function upgradeNetwork() {
  if [[ "$IMAGETAG" == *"1.4"* ]] || [[ $IMAGETAG == "latest" ]]; then
    docker inspect -f '{{.Config.Volumes}}' orderer.certificate.com | grep -q '/var/hyperledger/production/orderer'
    if [ $? -ne 0 ]; then
    ...
    if [ "${CERTIFICATE_AUTHORITIES}" == "true" ]; then
      COMPOSE_FILES="${COMPOSE_FILES} -f ${COMPOSE_FILE_CA}"
      export BYFN_CA1_PRIVATE_KEY=$(cd crypto-config/peerOrganizations/academy.certificate.com/ca && ls *_sk)
      export BYFN_CA2_PRIVATE_KEY=$(cd crypto-config/peerOrganizations/Student.certificate.com/ca && ls *_sk)
    fi
    if [ "${CONSENSUS_TYPE}" == "kafka" ]; then
      COMPOSE_FILES="${COMPOSE_FILES} -f ${COMPOSE_FILE_KAFKA}"
    elif [ "${CONSENSUS_TYPE}" == "etcdraft" ]; then
    ...
    docker-compose $COMPOSE_FILES stop orderer.certificate.com
    docker cp -a orderer.certificate.com:/var/hyperledger/production/orderer $LEDGERS_BACKUP/orderer.certificate.com
    docker-compose $COMPOSE_FILES up -d --no-deps orderer.certificate.com

    for PEER in peer0.academy.certificate.com peer1.academy.certificate.com peer0.Student.certificate.com peer1.Student.certificate.com; do
...
function networkDown() {
  # stop org3 containers also in addition to academy and Student, in case we were running sample to add org3
...
function json_explorer {
  sed -e "s/\${KEY}/$1/" certificate-verify-template.json
}

function replaceBlockChainExplorer() {
  CURRENT_DIR=$PWD
  cd crypto-config/peerOrganizations/academy.certificate.com/users/Admin@academy.certificate.com/msp/keystore/
  PRIV_KEY=$(ls *_sk)
  cd "$CURRENT_DIR"
  echo "$(json_explorer $PRIV_KEY)" > ../blockchain-explorer/app/platform/fabric/connection-profile/certificate-verify.json
}
...
function generateCerts() {
  ...
  echo
  echo "Generate CCP files for Academy and Student"
  ./ccp-generate.sh
}
...
# and two Peer Orgs (``Academy`` & ``Student``) each managing and maintaining two peer nodes.
...
# (``peer0.academy.certificate.com`` & ``peer0.student.certificate.com``).  Secondly, we point to
...
function generateChannelArtifacts() {
  ...
  echo
  echo "#################################################################"
  echo "#######    Generating anchor peer update for AcademyMSP   ##########"
  echo "#################################################################"
  set -x
  configtxgen -profile TwoOrgsChannel -outputAnchorPeersUpdate ./channel-artifacts/AcademyMSPanchors.tx -channelID $CHANNEL_NAME -asOrg AcademyMSP
  res=$?
  set +x
  if [ $res -ne 0 ]; then
