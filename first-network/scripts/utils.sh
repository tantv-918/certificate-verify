...
instantiateChaincode() {
  ...
  if [ -z "$CORE_PEER_TLS_ENABLED" -o "$CORE_PEER_TLS_ENABLED" = "false" ]; then
    ...
    set -x
    peer chaincode instantiate -o orderer.certificate.com:7050 --tls $CORE_PEER_TLS_ENABLED --cafile $ORDERER_CA -C $CHANNEL_NAME -n academy -l ${LANGUAGE} -v 1.0 -c '{"Args":[]}' -P "OR ('AcademyMSP.peer','StudentMSP.peer')" >&log.txt
    res=$?
    set +x
  fi
  cat log.txt
...
upgradeChaincode() {
  ...
  set -x
  peer chaincode upgrade -o orderer.certificate.com:7050 --tls $CORE_PEER_TLS_ENABLED --cafile $ORDERER_CA -C $CHANNEL_NAME -n academy -v 2.0 -c '{"Args":[]}' -P "AND ('AcademyMSP.peer','StudentMSP.peer','Org3MSP.peer')"
  res=$?
  set +x
  cat log.txt
